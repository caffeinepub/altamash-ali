...
import { Instagram, Youtube, Badge as BadgeIcon } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { Tabs, TabsList, TabsTrigger, TabsContent } from '@/components/ui/tabs';
import { pricingData, type ServiceCategory } from '../data/pricingData';
      ...
      <div className="bg-card gold-border rounded-2xl overflow-hidden card-glow">
        <div className="p-5 border-b border-border">
          <div className="flex items-center gap-3 mb-2">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isYT ? 'bg-red-500/10' : 'bg-pink-500/10'}`}>
              {isYT ? <Youtube className="w-5 h-5 text-red-400" /> : <Instagram className="w-5 h-5 text-pink-400" />}
            </div>
            <div>
              <h3 className="font-heading text-lg font-bold text-foreground">{service.name}</h3>
          ...
          <Tabs defaultValue={service.tiers[0].name}>
            <TabsList className="w-full mb-4 bg-surface">
              {service.tiers.map(tier => (
                <TabsTrigger key={tier.name} value={tier.name} className="flex-1 text-xs data-[state=active]:bg-gold data-[state=active]:text-background">
                  {tier.name}
                </TabsTrigger>
              ))}
            </TabsList>
              ...
              <TabsContent key={tier.name} value={tier.name}>
                <div className="space-y-2">
                  {tier.packages.map((pkg, i) => (
                    <div key={i} className="flex items-center justify-between py-2.5 px-3 rounded-lg hover:bg-accent/50 transition-colors group">
                      <span className="text-sm text-foreground">{pkg.quantity}</span>
                      <div className="flex items-center gap-3">
                        <span className="font-bold text-gold">₹{pkg.price.toLocaleString('en-IN')}</span>
                        <button
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-gold/10 border border-gold/30 rounded-full px-4 py-1.5 mb-4">
            ...
            <span className="text-sm text-gold font-medium">All Services & Pricing</span>
          </div>
          <h2 className="font-heading text-4xl md:text-5xl font-bold text-foreground mb-4">
            Choose Your <span className="gold-text">Growth Package</span>
          </h2>
          <p className="text-muted-foreground max-w-xl mx-auto">
            Affordable prices, fast delivery, and guaranteed results for Instagram and YouTube growth.
          </p>
        </div>
        ...
        <div className="flex justify-center gap-3 mb-10">
          {(['all', 'instagram', 'youtube'] as const).map(f => (
            ...
            >
              {f === 'all' ? '🌟 All' : f === 'instagram' ? '📸 Instagram' : '▶️ YouTube'}
            </button>
          ))}
        </div>
