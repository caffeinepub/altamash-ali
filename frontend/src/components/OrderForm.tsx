...
import { ShoppingCart, CheckCircle, ExternalLink, RotateCcw, Loader2 } from 'lucide-react';
      ...
      <section id="order" className="py-20 bg-background">
        <div className="container mx-auto px-4 max-w-2xl">
          <div className="bg-card gold-border rounded-2xl p-8 text-center card-glow">
            ...
            {form.orderId !== null && (
              <p className="text-sm text-gold mb-6">Order ID: #{form.orderId.toString()}</p>
            )}
            <div className="bg-surface rounded-xl p-4 mb-6 text-left space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Service:</span>
                <span className="text-foreground font-medium">{pricingData.find(s => s.id === form.serviceId)?.name}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Package:</span>
                <span className="text-foreground font-medium">{form.packageLabel}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Amount:</span>
                <span className="text-gold font-bold">₹{form.price.toLocaleString('en-IN')}</span>
              </div>
            </div>
            <p className="text-sm text-muted-foreground mb-6">
              WhatsApp has been opened with your order details. Please complete the payment and send the screenshot to confirm your order.
            </p>
      ...
      <div className="container mx-auto px-4 max-w-2xl">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 bg-gold/10 border border-gold/30 rounded-full px-4 py-1.5 mb-4">
            ...
            <span className="text-sm text-gold font-medium">Place Your Order</span>
          </div>
        ...
        <form onSubmit={handleSubmit} className="bg-card gold-border rounded-2xl p-6 md:p-8 card-glow space-y-6">
          {/* Service Selection */}
          <div className="space-y-2">
            <Label className="text-foreground font-medium">Select Service *</Label>
            <Select value={form.serviceId} onValueChange={form.handleServiceChange}>
              <SelectTrigger className="bg-surface border-border text-foreground h-12">
                <SelectValue placeholder="Choose a service..." />
              </SelectTrigger>
              <SelectContent className="bg-card border-border">
                {pricingData.map(service => (
                  <SelectItem key={service.id} value={service.id} className="text-foreground hover:bg-accent">
                    {service.icon} {service.name}
                    {service.badge && <span className="ml-2 text-xs text-gold">({service.badge})</span>}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          {/* Package Selection */}
          <div className="space-y-2">
            <Label className="text-foreground font-medium">Select Package *</Label>
            <Select
            ...
            >
              <SelectTrigger className="bg-surface border-border text-foreground h-12">
                <SelectValue placeholder={form.serviceId ? 'Choose a package...' : 'Select a service first'} />
              </SelectTrigger>
              <SelectContent className="bg-card border-border">
                {form.availablePackages.map((pkg, i) => (
                  <SelectItem key={i} value={pkg.label} className="text-foreground hover:bg-accent">
                    <span>{pkg.label}</span>
                    <span className="ml-2 text-gold font-bold">₹{pkg.price.toLocaleString('en-IN')}</span>
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          {/* Profile/Video URL */}
          <div className="space-y-2">
            <Label className="text-foreground font-medium">Profile / Video URL *</Label>
            <Input
              type="url"
              ...
              required
            />
            <p className="text-xs text-muted-foreground">Paste your Instagram profile link or YouTube video/channel link</p>
          </div>

          {/* WhatsApp Number (optional) */}
          <div className="space-y-2">
            <Label className="text-foreground font-medium">Your WhatsApp Number (Optional)</Label>
            <Input
              type="tel"
          ...
          {form.price > 0 && (
            <div className="bg-gold/5 border border-gold/20 rounded-xl p-4 flex items-center justify-between">
              ...
              <span className="text-2xl font-bold text-gold">₹{form.price.toLocaleString('en-IN')}</span>
            </div>
          )}
          ...
          <p className="text-xs text-muted-foreground text-center">
            After clicking, WhatsApp will open with your order details. Complete payment via UPI QR code and send the screenshot.
          </p>
        </form>
      </div>
    </section>
