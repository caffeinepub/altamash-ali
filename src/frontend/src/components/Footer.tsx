import { CreditCard, Heart, MessageCircle } from "lucide-react";
    ...
    <footer id="contact" className="bg-card border-t border-border">
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
          {/* Brand */}
          <div>
            <h3 className="font-heading text-2xl font-bold gold-text mb-2">
              Crown Followers
            </h3>
            <p className="text-muted-foreground text-sm leading-relaxed mb-4">
              Your trusted Social Media Growth Partner. We provide real
              Instagram and YouTube growth services at the best prices in India.
            </p>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-muted-foreground">
              {[
                { icon: "📸", label: "Instagram Followers" },
                { icon: "❤️", label: "Instagram Likes" },
                { icon: "▶️", label: "YouTube Views" },
                { icon: "🔔", label: "YouTube Subscribers" },
                { icon: "🎬", label: "YouTube Shorts Views" },
                { icon: "👍", label: "YouTube Likes" },
              ].map((item) => (
                <li key={item.label}>
                  <button
                    type="button"
                    className="hover:text-gold transition-colors cursor-pointer text-left w-full"
                    onClick={() =>
                      document
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium text-gold font-mono">
                    7668237595@fam
                  </p>
                </div>
              </div>
        ...
        <div className="border-t border-border pt-6 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-muted-foreground">
          ...
          <p className="flex items-center gap-1">
            Built with{" "}
            <Heart className="w-3.5 h-3.5 text-gold fill-gold mx-0.5" /> using{" "}
            <a
